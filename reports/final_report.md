...
- **Fan-out (Detectives):** In `src/graph.py`, `build_audit_graph()` adds edges from START to `repo_investigator`, `doc_analyst`, and `vision_inspector` so all three run in parallel. Each returns partial `evidences` (reducer: `merge_evidences` in `src/state.py`).
...
- **Fan-out (Judges):** From `judicial_entry`, edges go to `prosecutor`, `defense`, and `tech_lead` so all three Judges run in parallel. Each returns partial `opinions` (reducer: `merge_opinions` in `src/state.py`).
...
1. **START → Detectives:** `repo_url`, `pdf_path`, `rubric_dimensions` are in state. Each Detective filters dimensions by `target_artifact`, invokes tools (clone_repo, ingest_pdf, analyze_graph_structure, etc.), and emits `Evidence` into `state["evidences"]` keyed by dimension id. LangGraph merges partial dicts via the `merge_evidences` reducer in `src/state.py`.
...
```
        ...
        └─────────┬─────────┘       └─────────┬─────────┘       └─────────┬─────────┘
                  ...
                  │ (merge_evidences)         │ (merge_evidences)          │ (merge_evidences)
                  └───────────────────────────┼───────────────────────────┘
                                              │
                    ┌─────────────────────────▼─────────────────────────┐
...
flowchart TB
    ...
    RI -->|evidences merge_evidences| AGG[EvidenceAggregator]
    DA --> AGG
    VI --> AGG
    ...
    JE --> D[Defense]
    JE --> T[Tech Lead]
    P -->|opinions merge_opinions| JC[Judge Collector]
    D --> JC
    T --> JC
    JC --> CJ[Chief Justice]
    CJ --> END2([END])
...
- **AST over regex for graph structure:** Regex fails on multiline or nested structures; AST in `analyze_graph_structure()` in `src/tools/repo_tools.py` parses `src/graph.py` and detects StateGraph, add_edge, add_node, and reducer usage accurately. **Trade-off:** Python-only; multi-language would need tree-sitter or similar.
